...
MeshTest::MeshTest () :
  ...
  m_inf_prefix("/home/mesh/app/ns-allinone-3.19/ns-3.19/hadi_report/AggRcpPC/Run1")
{
}
void
MeshTest::Configure (int argc, char *argv[])
{
  ...
  cmd.AddValue ("inf_prefix",  "prefix of input file", m_inf_prefix);
  cmd.AddValue ("RANDOM_SEED", "Seed", m_RANDOM_SEED);
  cmd.AddValue ("NUM_CONNS", "Seed", m_num_connections);
...
MeshTest::InstallApplication ()
{
	...
	std::cout << "RngRun=" << (int)rr.Get() << std::endl;
	UniformVariable rng = UniformVariable();
	double du_time = 50;
	double st_time = 21;
	uint8_t node_src_num;

	int connection_Number=1500 + m_num_connections;
	...
	for (int i = 0; connection_Number > 0 ; ++i) {
		...
		if (st_time  + du_time > m_totalTime){
			continue;
		}
		else
		{
		...
		if(connection_Number >1500){
			node_src_num= rng.GetInteger(1,m_xSize*m_ySize-1);
		}else{
			node_src_num= rng.GetInteger(5,m_xSize*m_ySize-1);
		}
...
//				std::cout << array[position] << "  AAAAAAAA" << std::endl ;
		...
		{
			std::cout << "File could not be opened." << os << std::endl;
		}
...
MeshTest::Run ()
{
  ...
  std::cout << m_xml_prefix << " " << (int)rr.Get() << " " << m_root << std::endl;
  Simulator::Run ();
