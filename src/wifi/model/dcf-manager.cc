...
: m_dcf (dcf)
{
	...
	{
		RxStart =Simulator::Now();
		m_dcf->NotifyRxStartNow (duration);
	}
	virtual void NotifyRxEndOk (void)
	{
		if((Simulator::Now() < Seconds(71)) && (Simulator::Now() > Seconds(21))){
					m_dcf->AddAllStateDuration(Simulator::Now()-RxStart);
				}
	...
	{
		if((Simulator::Now() < Seconds(71)) && (Simulator::Now() > Seconds(21))){
			m_dcf->AddAllStateDuration(duration);
		}
...
void
DcfManager::AddAllStateDuration (Time t){
	sumTime+=t;
}
Time
DcfManager::GetAllStateDuration (){
	return sumTime;
}
